...
const makeArray = (size: number): number[] => new Array(size).fill(0).map((_, index) => index);
...
export const GameProvider: React.FC<Size & { bombs: number }> = ({
  ...
  const resetGame = useCallback(() => {
    ...
    setItems(
      makeArray(rows)
        .map((row) => makeArray(columns).map((column) => ({
  ...
  useEffect(() => {
    ...
    if (actions.length > 0) {
      ...
      const interactions: { [key: number]: BoxStates } = {};
      for (const { coordinate, action } of actions) {
        ...
        if (
          ...
          const n = neighbors(indexToCoord(index, size), size).map((it) => coordToIndex(it, size));
          ...
          if (flagged.length === items[index].neighborCount) {
            if (bombs.length > 0) {
              ...
              covered.forEach((it) => handleLeftClick(it, interactions, size, items));
            }
          }
        }
      }
      if (newState !== GameStates.ERROR) {
        const actionedItems = newItems.map((it, index) => (index in interactions && it.state !== interactions[index]
  ...
  const notBombs = useMemo(
    () => items.filter(({ state }) => [BoxStates.UNCOVERED, BoxStates.UNKNOWN].includes(state)).length,
    [items],
  );
  useEffect(() => {
  ...
  return (
    <GameContext.Provider value={{
      items, size, state, bombsLeft, resetGame,
    }}
    >
      {children}
